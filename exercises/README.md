...
flowchart TD
    ...
    install([1. install]) --> tfidf([2. TFIDF])
    tfidf --> webui([3. Web UI])
    webui --> eval([4. evaluate])
    eval --> mllm([5. MLLM])
    mllm --> ensemble([6. ensemble])
    ensemble --> nn_ensemble([6-1. NN ensemble])
    ensemble --> custom([6-2. Custom corpus])
    ensemble --> dvc([6-3. DVC])
    mllm --> ft([5-1. Hogwarts/fastText])
    mllm --> lang_filter([5-2. Languages & filtering])
    webui --> rest([3-1. REST API])
    rest --> production([3-2. Production use])
    eval --> omikuji([4-1. Omikuji])
    omikuji --> classification([4-2. Classification])
    class install core
    class tfidf core
    class webui core
    class eval core
...
- [exercise](/exercises/12_langfilter.md)
